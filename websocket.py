...
server = "censored"  # Server IP Address or domain eg: tabvn.com
        ...
        if is_bool:
            ...
            if streaming_process is None:
                ...
                streaming_process = subprocess.Popen(
                    ffmpeg_command, shell=True, stdin=subprocess.PIPE)
...
class AppProtocol(WebSocketClientProtocol):
    ...
    def onOpen(self):
        ...
        def hello_server():
            message = {"action": "pi_online", "payload": {
                "id": "tabvn", "secret": "key"}}
    ...
    def onMessage(self, payload, isBinary):
        if (isBinary):
            ...
            print("Got Text message from the server {0}".format(
                payload.decode('utf8')))
...
if __name__ == '__main__':
    ...
    # This url is specifically required by the boiler plate code
    # TODO: streamline this url
    factory = AppFactory(u"ws://{0}/websockets?userid=999".format(server).format(":").format(port))
